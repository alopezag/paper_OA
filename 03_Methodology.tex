...
Speech, which is a prevalent way to interact with DAs such as Amazon Alexa and Google Assistent, is characterized by high efficiency, naturalness, low cognitive load and hands-free capabilities. On the downside, speech is hard in a noisy environment or when multiple persons are working in close proximity~\cite{hou2018VisualFeedbackDesign}. Speech, however, can be complemented by graphical user interfaces (GUIs) and other modalities (e.g., touch-based or gestural interaction) to query a DA~\cite{belkadi2020IntelligentAssistantSystem,klopfenstein2017RiseBotsSurvey,heller2019TaskHerderWearableMinimal}. 


Leveraging multiple modalities and devices in combination with a conversational UI allows seamless interaction~\cite{white2019MultideviceDigitalAssistance}, as well as taking advantage of fitting technologies in different situations~\cite{heinz2018MultideviceAssistiveSystem}. 

Therefore, we will support multiple devices and modalities to interact with the DA, using web-based multi-device formats. 
